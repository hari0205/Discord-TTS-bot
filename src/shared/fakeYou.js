...
const { fakeYouToken } = require("../../config.json"); // optional
...
function requestVoiceList() {
	return new Promise(async(resolve, reject) => {
		...
		const response = await fetchPatiently("https://api.fakeyou.com/tts/list", {
			method: "GET",
			headers: {
				"Authorization": fakeYouToken, //Optional 
				"Accept": "application/json"
			}
		}).catch(reject);
		if (!response.ok) return;
...
// Wait 5 minutes before attempting to cache again
const minutesBetweenCaches = 5;
